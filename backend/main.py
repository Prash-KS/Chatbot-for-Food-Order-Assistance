...
async def remove_from_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
        ...
        food_items = parameters["food-entity"]
...
async def save_to_db(order: dict):
    next_order_id = await db_helper.get_next_order_id()  # Await async function

    for food_item, quantity in order.items():
        rcode = await db_helper.insert_order_item(  # Await async function
            food_item,
            quantity,
            next_order_id
        )
...
async def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
        ...
        order_id = await save_to_db(order)  # Await async function

        if order_id == -1:
            fulfillment_text = "Sorry, I couldn't process your order due to backend error. " \
                                "Please place a new order again"
        else:
            order_total = await db_helper.get_total_order_price(order_id)  # Await async function
...
async def track_order(parameters: dict, session_id: str):
    ...
    order_status = await db_helper.get_order_status(order_id)  # Await async function
